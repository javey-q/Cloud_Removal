...
from arch.submodules import df_conv, df_resnet_block, kernel2d_conv
...
class RDN_residual_CR(nn.Module):
    def __init__(self, opt):
        ...
        input_size = opt['fig_size']
        opt_channel = opt['opt_channel']
        ...
        self.SFENet1 = nn.Conv2d(opt_channel * 4, self.G0, 5, padding=2, stride=1)
        self.SFENet2 = nn.Conv2d(self.G0, self.G0, kSize, padding=(kSize - 1) // 2, stride=1)

        self.SFENet1_SAR = nn.Conv2d(sar_channel * 4, self.G0, 5, padding=2, stride=1)
        ...
        for i in range(self.D):
            self.DF.append(DFG(channels*2, self.ks_2d))
        ...
        if self.use_gray:
            self.UPNet_SAR = nn.Sequential(*[
                nn.Conv2d(self.G0, 256, kSize, padding=(kSize - 1) // 2, stride=1),
                nn.PixelShuffle(2),
                nn.Conv2d(64, 1, kSize, padding=(kSize - 1) // 2, stride=1)
            ])
        ...
        self.UPNet = nn.Sequential(*[
            nn.Conv2d(self.G0, 256, kSize, padding=(kSize - 1) // 2, stride=1),
            nn.PixelShuffle(2),
            nn.Conv2d(64, opt_channel, kSize, padding=(kSize - 1) // 2, stride=1)
        ])
    ...
    def forward(self, cloudy_data, SAR):
        ...
        for i in range(self.D):
            ...
            if not self.use_gray and  i == self.D - 1:
                x = self.fuse(x, x_SAR, i, last_layer=True)
            else:
                x, x_SAR = self.fuse(x, x_SAR, i)

            RDBs_out.append(x)
            if self.use_gray:
                RDBs_out_SAR.append(x_SAR)
        ...
        if self.use_gray:
            x_SAR = self.GFF_SAR(torch.cat(RDBs_out_SAR, 1))
            x_SAR += f__1__SAR
            pred_gray_data = self.UPNet_SAR(x_SAR)
            return pred_CloudFree_data, pred_gray_data

        return pred_CloudFree_data

    def fuse(self, OPT, SAR, i, last_layer=False):
...
if __name__ == "__main__":
    ...
    opt_network['fig_size'] = 64

    model = RDN_residual_CR(opt_network).cuda()

    cloudy = torch.rand(1, 3, 64, 64).cuda()
    cloudfree = torch.rand(1, 3, 64, 64).cuda()
    s1_sar = torch.rand(1, 2, 64, 64).cuda()
